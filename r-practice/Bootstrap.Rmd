...
x <- dataxy[,1]
y <- dataxy[,2]
alpha[i] <- (var(y)-cov(x, y))/(var(x)+var(y)-2*cov(x,y)) #estimate of alpha (1000 times)
}
alphahat <- mean(alpha)
stderr   <- sd(alpha)  
...
library(hrbrthemes)
par(mfrow=c(1,2))
hist(alpha, col="orange", border="white", xlab=bquote(alpha), main="True")
